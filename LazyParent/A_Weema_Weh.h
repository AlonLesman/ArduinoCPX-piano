...
/* 
  ...
  imported from https://github.com/robsoncouto/arduino-songs                                            
...
void LionsSong() {
  for (int thisNoteB = 0; thisNoteB < notesB * 2; thisNoteB = thisNoteB + 2) {
    dividerB = melodyB[thisNoteB + 1];
    if (dividerB > 0) {
    ...
    CircuitPlayground.playTone(melodyB[thisNoteB] * 1.9, noteDurationB * 0.6);
    delay(noteDurationB / 4);
    noTone(buzzerB * 1.5);
  }
}
